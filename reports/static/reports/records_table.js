...
$(document).ready(function() {
    ...
    var dt_table = $('.datatable').dataTable({
        ...
        buttons: [
            {
                ...
                customize : function (doc) {
                    ...
                    doc['footer'] = (function(page, pages) {
                        return {
                            columns: [
                                {
                                    alignment: 'left',
                                    text: 'Wygenerowano: ' + moment().format("DD-MM-YYYY")
                                },
                                {
                                    alignment: 'right',
                    ...
                    if (days) {
                        ...
                        formTable.push({ ...pdfLine, text: 'Do: ' + days[1]});
                    }

                    for (let [i, value] of ['Pracownik: ', 'Stanowisko: ', 'Budowa: '].entries()) {
                        chosenValue = yadcf.exGetColumnFilterVal(dt_table, i)
                        if(chosenValue)
                            formTable.push({ ...pdfLine, text: value + chosenValue})
                    }
        ...
        ajax: {
            url: records_json,
            data: function(data) {
                data.minDate = startdate.format("YYYY-MM-DD")
                data.maxDate = enddate.format("YYYY-MM-DD")
                return data;
            }
        }
    });
